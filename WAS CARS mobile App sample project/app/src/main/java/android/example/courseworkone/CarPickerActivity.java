...
public class CarPickerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        i1 = findViewById(R.id.identify_car);
        identifyCar = findViewById(R.id.button_submit);

        correctCar = getCar(random);
        i1.setImageResource(cars[random]);       //selecting random image from the arrays
        ...
        spi = findViewById(R.id.select_car);  //select the spinner
        if (spi != null) {
            spi.setAdapter(names);   //adding names array to the spinner
        }
    ...
    public void clickedSubmit(View view) {
        ...
        if(!clickedSubmit) {
            String text = spi.getSelectedItem().toString();
